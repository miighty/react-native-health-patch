...
- (void)body_AppleSleepingWristTemperature:(NSDictionary *)input callback:(RCTResponseSenderBlock)callback
{
    HKQuantityType *appleSleepingWristTemperature = [HKQuantityType quantityTypeForIdentifier:HKQuantityTypeIdentifierAppleSleepingWristTemperature];
    ...
    [self fetchQuantitySamplesOfType:appleSleepingWristTemperature
                                unit:unit
                           predicate:predicate
                           ascending:ascending
                               limit:limit
                          completion:^(NSArray *results, NSError *error) {
                              if(results){
                                  ...
                                  NSLog(@"error getting apple wrist temperature samples: %@", error);
                                  callback(@[RCTMakeError(@"error getting apple wrist temperature samples:", error, nil)]);
                                  return;
                              }
                          }];
}
